...
fn test_parse_suricata_alerts() {
    ...
    let alerts = main::parse_suricata_alerts(&sample_path).expect("Failed to parse Suricata alerts");
    
    // We should have 6 alerts (one entry is a flow, not an alert)
    assert_eq!(alerts.len(), 6, "Expected 6 alerts but found {}", alerts.len());
    ...
    // Check correct protocol parsing
    assert_eq!(alerts[0].protocol, "TCP");
    assert_eq!(alerts[4].protocol, "UDP");
    assert_eq!(alerts[5].protocol, "ICMP");
    
    // Check IP addresses
    let expected_ip = IpAddr::V4(Ipv4Addr::new(192, 168, 1, 100));
    assert_eq!(alerts[0].src_ip, expected_ip);
}
...
fn test_correlate_alerts_with_flows() {
    ...
    assert!(!flow.alerts.is_empty(), "Flow should have an alert attached");
    assert_eq!(flow.alerts[0].signature, "ET SCAN Potential SSH Scan");
    
    // Check if attack label was set
    assert!(flow.attack_label.is_some());
    assert_eq!(flow.attack_label.as_ref().unwrap(), "attack:Suricata Alert");
}
...
fn test_handle_missing_fields() {
    ...
    assert_eq!(alerts.len(), 2, "Expected 2 alerts even with missing fields");
    
    // First alert should have defaults for alert fields
    assert_eq!(alerts[0].signature, "Unknown");
    assert_eq!(alerts[0].signature_id, 0);
    assert_eq!(alerts[0].severity, 0);
    
    // Second alert should have the signature but default values for missing fields
