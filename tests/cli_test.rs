...
fn test_basic_cli_functionality() {
    ...
    if !test_pcap_path.exists() {
        println!("Skipping CLI test - test PCAP file not found at {:?}", test_pcap_path);
        return;
    }
    ...
    let has_csv = output_files.filter_map(Result::ok)
        .any(|entry| {
...
fn test_json_output() {
    ...
    if !test_pcap_path.exists() {
        println!("Skipping JSON output test - test PCAP file not found at {:?}", test_pcap_path);
        return;
    }
    ...
    let output_files = fs::read_dir(output_dir).expect("Failed to read output directory");
    let has_json = output_files.filter_map(Result::ok)
        .any(|entry| {
...
fn test_labels() {
    ...
    if !test_pcap_path.exists() {
        println!("Skipping labels test - test PCAP file not found at {:?}", test_pcap_path);
        return;
    }
