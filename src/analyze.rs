...
impl AttackType {
    fn as_str(&self) -> &'static str {
        match self {
            AttackType::TcpSynScan => "TCP SYN Scan",
            AttackType::TcpConnectScan => "TCP Connect Scan",
            AttackType::UdpScan => "UDP Scan",
            AttackType::SynFlood => "SYN Flood",
            AttackType::PortSweep => "Port Sweep",
            AttackType::PingSweep => "Ping Sweep",
            AttackType::SuricataAlert => "Suricata Alert",
        }
    }
}
...
pub fn run_with_cli(cli: AnalyzeCli) -> Result<(), Box<dyn Error>> {
    ...
    for pkt in &mut all_packets {
        if let (Some(src_ip), Some(dst_ip), Some(src_port), Some(dst_port), Some(protocol)) = (
            ...
            pkt.dst_port,
            pkt.protocol,
        ) && let Some(key) = FlowKey::new(src_ip, dst_ip, src_port, dst_port, protocol)
        {
    ...
    for pkt in &mut all_packets {
        if let Some(fid) = &pkt.flow_id
            && let Some(lab) = flow_labels.get(fid)
        {
...
pub fn run_from_args(args: Vec<String>) -> Result<(), Box<dyn Error>> {
    if args.get(1).map(|arg| arg == "rewrite").unwrap_or(false) {
        let mut rewrite_args = vec![args[0].clone()];
        rewrite_args.extend_from_slice(&args[2..]);
        let rewrite_cli = rewrite::RewriteCli::parse_from(rewrite_args);
        return rewrite::run(rewrite_cli);
    }

    let analyze_cli = if args.get(1).map(|arg| arg == "analyze").unwrap_or(false) {
        let mut trimmed = vec![args[0].clone()];
        trimmed.extend_from_slice(&args[2..]);
        AnalyzeCli::parse_from(trimmed)
    } else {
        AnalyzeCli::parse_from(args)
    };

    run_with_cli(analyze_cli)
}
    ...
    for (src_ip, stats) in source_stats {
        ...
        if config.heuristics.syn_scan.enabled {
            for (dst_ip, port_set) in &stats.dst_ports {
                ...
                if attack_flows.values().any(|attack_type| {
                    attack_type == AttackType::TcpSynScan.as_str()
                   ...
                   port_set.len() >= scan_threshold
                {
                    ...
                    if let Some(syn_count) = stats.syn_packets.get(dst_ip)
                        && *syn_count >= port_set.len() as u32 / 2
                    {
        ...
        if config.heuristics.udp_scan.enabled {
            for (dst_ip, port_set) in &stats.dst_ports {
                // Skip if we've already detected a scan for this source->dest pair
                if attack_flows.values().any(|attack_type| {
                    attack_type == AttackType::UdpScan.as_str()
                        && find_flow_id_for_scan(src_ip, *dst_ip, flows).is_some()
                }) {
                    continue;
                }
        ...
        if config.heuristics.port_sweep.enabled {
            ...
            for (port, dst_ips) in &stats.dst_ips_per_port {
                ...
                if let Some(dst_ip) = dst_ips.iter().next()
                    && let Some(flow_id) = find_flow_id_for_service(src_ip, *dst_ip, *port, flows)
                {
        ...
        if config.heuristics.ping_sweep.enabled {
            ...
            if stats.ping_packets.len() >= ping_sweep_threshold {
                ...
                if let Some(dst_ip) = stats.ping_packets.keys().next()
                    && let Some(flow_id) = find_flow_id_for_ping(src_ip, *dst_ip, flows)
                {
...
pub fn parse_suricata_alerts(path: &Path) -> Result<Vec<SuricataAlert>, Box<dyn Error>> {
    ...
    let alerts = Mutex::new(Vec::<(usize, SuricataAlert)>::new());
    ...
    let mut result_pairs = match alerts.into_inner() {
        Ok(alerts) => alerts,
        Err(e) => return Err(format!("Mutex error: {:?}", e).into()),
    };

    result_pairs.sort_by_key(|(idx, _)| *idx);
    let result: Vec<SuricataAlert> = result_pairs.into_iter().map(|(_, alert)| alert).collect();
