...
use std::str::FromStr;
use std::sync::{Mutex, atomic::{AtomicUsize, Ordering as AtomicOrdering}};
...
#[derive(Parser, Debug)]
#[command(name = "pcap-sleuth", version, author, about = "A tool to process PCAP files and extract network flow data")]
struct Cli {
    /// Input PCAP file
...
impl FlowKey {
    pub fn new(src_ip: IpAddr, dst_ip: IpAddr, src_port: u16, dst_port: u16, protocol: u8) -> Option<Self> {
        if protocol == 6 || protocol == 17 {
            match src_ip.cmp(&dst_ip) {
                Ordering::Less => Some(FlowKey { ip_a: src_ip, ip_b: dst_ip, port_a: src_port, port_b: dst_port, protocol }),
                Ordering::Greater => Some(FlowKey { ip_a: dst_ip, ip_b: src_ip, port_a: dst_port, port_b: src_port, protocol }),
                Ordering::Equal => {
                    if src_port <= dst_port {
                        Some(FlowKey { ip_a: src_ip, ip_b: dst_ip, port_a: src_port, port_b: dst_port, protocol })
                    } else {
                        Some(FlowKey { ip_a: dst_ip, ip_b: src_ip, port_a: dst_port, port_b: src_port, protocol })
                    }
                }
            }
        } else {
    ...
    pub fn to_string_id(&self) -> String {
        format!("{}-{}:{}-{}:{}", self.protocol, self.ip_a, self.port_a, self.ip_b, self.port_b)
    }
}
...
fn write_packet_csv(packets: &[PacketInfo], output_path: &Path) -> Result<(), Box<dyn Error>> {
    ...
    wtr.write_record(&["timestamp","src_ip","dst_ip","src_port","dst_port","protocol","length","payload_length","tcp_flags","icmp_type","icmp_code","flow_id","label"])?;
    for pkt in packets {
        wtr.write_record(&[
...
fn write_flow_json(flows: &HashMap<FlowKey, FlowData>, output_path: &Path) -> Result<(), Box<dyn Error>> {
...
impl Default for HeuristicConfig {
    fn default() -> Self {
        Self {
            syn_scan: ScanConfig { enabled: default_true(), threshold: default_scan_threshold() },
            connect_scan: ScanConfig { enabled: default_true(), threshold: default_scan_threshold() },
            udp_scan: ScanConfig { enabled: default_true(), threshold: default_udp_scan_threshold() },
            syn_flood: SynFloodConfig::default(),
            port_sweep: PortSweepConfig::default(),
            ping_sweep: ScanConfig { enabled: default_true(), threshold: default_ping_sweep_threshold() },
        }
    }
}
...
fn default_suricata_alert_format() -> String { "suricata:{alert_category}:{signature}".to_string() }

// Default value helper functions
fn default_true() -> bool { true }
fn default_scan_threshold() -> usize { 20 }
fn default_udp_scan_threshold() -> usize { 15 } // Lower threshold for UDP scans
fn default_attack_label_format() -> String { "attack:{type}".to_string() }
fn default_tcp_timeout() -> u64 { 300 } // 5 minutes
fn default_udp_timeout() -> u64 { 180 } // 3 minutes
fn default_check_interval() -> u64 { 1000 } // Check every 1000 packets
fn default_syn_flood_threshold() -> u32 { 100 } // 100 SYNs per second
fn default_syn_flood_window() -> u64 { 1 } // 1 second time window
fn default_port_sweep_threshold() -> usize { 10 } // 10 distinct destination IPs
fn default_ping_sweep_threshold() -> usize { 5 } // 5 distinct destination IPs for ping sweep
...
fn parse_packet(data: &[u8], ts: DateTime<Utc>, length: u32) -> PacketInfo {
    let mut pkt = PacketInfo { timestamp: ts, src_ip: None, dst_ip: None,
        src_port: None, dst_port: None, protocol: None, length,
        payload_length: 0, tcp_flags: None, icmp_type: None, icmp_code: None, flow_id: None, label: None };
            ...
            EtherTypes::Ipv4 => {
                if let Some(ip_pkt) = Ipv4Packet::new(eth_pkt.payload()) {
                    ...
                    if proto == IpNextHeaderProtocols::Tcp {
                        ...
                        } else { warn!("Failed to parse TCP packet"); }
                        ...
                        } else { warn!("Failed to parse UDP packet"); }
                    } else if proto == IpNextHeaderProtocols::Icmp {
                        if let Some(icmp_pkt) = IcmpPacket::new(ip_pkt.payload()) {
                            pkt.protocol = Some(1); // ICMP
                            pkt.icmp_type = Some(icmp_pkt.get_icmp_type());
                            pkt.icmp_code = Some(icmp_pkt.get_icmp_code());
                            pkt.payload_length = icmp_pkt.payload().len();
                        } else { warn!("Failed to parse ICMP packet"); }
                    }
                } else { warn!("Failed to parse IPv4 packet"); }
            }
            EtherTypes::Ipv6 => {
                if let Some(ip6_pkt) = Ipv6Packet::new(eth_pkt.payload()) {
                    ...
                    if proto == IpNextHeaderProtocols::Tcp {
                        if let Some(tcp_pkt) = TcpPacket::new(ip6_pkt.payload()) {
                            ...
                            pkt.tcp_flags = Some(tcp_pkt.get_flags());
                            pkt.payload_length = tcp_pkt.payload().len();
                        } else { warn!("Failed to parse TCPv6 packet"); }
                    } else if proto == IpNextHeaderProtocols::Udp {
                        if let Some(udp_pkt) = UdpPacket::new(ip6_pkt.payload()) {
                            pkt.src_port = Some(udp_pkt.get_source());
                            pkt.dst_port = Some(udp_pkt.get_destination());
                            pkt.payload_length = udp_pkt.payload().len();
                        } else { warn!("Failed to parse UDPv6 packet"); }
                        ...
                        if let Some(icmpv6_pkt) = Icmpv6Packet::new(ip6_pkt.payload()) {
                            ...
                            pkt.icmp_type = Some(icmpv6_pkt.get_icmpv6_type());
                            pkt.icmp_code = Some(icmpv6_pkt.get_icmpv6_code());
                            pkt.payload_length = icmpv6_pkt.payload().len();
                        } else { warn!("Failed to parse ICMPv6 packet"); }
                    }
                } else { warn!("Failed to parse IPv6 packet"); }
            }
            _ => { debug!("Unsupported EtherType: {:?}", eth_pkt.get_ethertype()); }
        }
    } else { warn!("Failed to parse Ethernet packet"); }
    pkt
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if detect_attacks {
        ...
        info!("SYN scan detection: {}", config.detection.heuristics.syn_scan.enabled);
        info!("Connect scan detection: {}", config.detection.heuristics.connect_scan.enabled);
        info!("UDP scan detection: {}", config.detection.heuristics.udp_scan.enabled);
        info!("SYN flood detection: {}", config.detection.heuristics.syn_flood.enabled);
        info!("Port sweep detection: {}", config.detection.heuristics.port_sweep.enabled);
        info!("Ping sweep detection: {}", config.detection.heuristics.ping_sweep.enabled);
    }
    ...
    while let Ok(packet) = cap.next_packet() {
        ...
        let ts = match Utc.timestamp_opt(sec, (usec * 1000) as u32).single() {
            Some(t) => t,
            None => {
                warn!("Invalid timestamp in packet {}: sec={}, usec={}. Skipping packet.", count, sec, usec);
                continue;
            }
        };
                ...
                for (key, data) in &mut flows {
                    if key.to_string_id() == *flow_id {
                        ...
                        if data.label.is_none() {
                            let label = config.detection.labels.attack_format
    ...
    if let Some(suricata_alerts_path) = &cli.suricata_alerts {
        if config.suricata.enabled {
            ...
            let matched = correlate_suricata_alerts(&mut flows, &alerts, &config.suricata);
    ...
    if !cli.csv && !cli.json {
        warn!("No output format specified (use --csv and/or --json). Processing complete, but no files generated.");
    }
...
fn parse_label_rules(path: &Path) -> Result<Vec<LabelRule>, Box<dyn Error>> {
    ...
    for (idx, result) in rdr.records().enumerate() {
        ...
        if record.len() < 3 {
            warn!("Skipping invalid label rule at line {}: expected 3 fields, got {}", idx + 1, record.len());
            continue;
        }
        ...
        if lab.is_empty() {
            warn!("Skipping label rule at line {}: label cannot be empty", idx + 1);
            continue;
        }
        match typ {
            "ip_addr" => {
                match IpAddr::from_str(crit) {
                    Ok(ip) => rules.push(LabelRule { rule: LabelType::IpAddr(ip), label: lab.clone() }),
                    Err(e) => warn!("Invalid IP '{}' in label rule at line {}: {}", crit, idx + 1, e),
                }
            }
            "flow_id" => {
                rules.push(LabelRule { rule: LabelType::FlowId(crit.to_string()), label: lab.clone() });
            }
            "subnet" => {
                match IpNetwork::from_str(crit) {
                    Ok(net) => rules.push(LabelRule { rule: LabelType::Subnet(net), label: lab.clone() }),
                    Err(e) => warn!("Invalid subnet '{}' in label rule at line {}: {}", crit, idx + 1, e),
                }
            }
            "port" => {
                match crit.parse::<u16>() {
                    Ok(p) => rules.push(LabelRule { rule: LabelType::Port(p), label: lab.clone() }),
                    Err(e) => warn!("Invalid port '{}' in label rule at line {}: {}", crit, idx + 1, e),
                }
            }
            _ => { warn!("Unknown label type '{}' at line {}", typ, idx + 1); }
        }
    }
    Ok(rules)
}
...
fn run_attack_detection(
    ...
    for pkt in packets {
        if let (Some(src_ip), Some(dst_ip), Some(dst_port), Some(protocol)) = 
            ...
            stats.dst_ips_per_port.entry(dst_port).or_default().insert(dst_ip);
            ...
            if protocol == 6 { // TCP
                if let Some(tcp_flags) = pkt.tcp_flags {
                    ...
                    if ack_flag && !syn_flag {
                        ...
                        stats.successful_connections.entry(dst_ip).or_default().insert(dst_port);
                    }
                }
            }
            ...
            if protocol == 6 { // TCP
                if let Some(tcp_flags) = pkt.tcp_flags {
                    ...
                    if tcp_flags & 0x02 != 0 {
                        source_stats.entry(src_ip).or_default().syn_timestamps.entry((dst_ip, dst_port)).or_default().push(pkt.timestamp);
                    }
                }
            }
            ...
            if protocol == 1 { // ICMP
                ...
                if let (Some(icmp_type), Some(icmp_code)) = (pkt.icmp_type, pkt.icmp_code) {
                    if icmp_type == 8 && icmp_code == 0 { // Echo Request (ping)
                        *stats.ping_packets.entry(dst_ip).or_default() += 1;
                    }
                }
            } else if protocol == 58 { // ICMPv6
                // ICMPv6 Echo Request is type 128
                if let (Some(icmp_type), Some(icmp_code)) = (pkt.icmp_type, pkt.icmp_code) {
                    if icmp_type == 128 && icmp_code == 0 { // Echo Request (ping)
                        *stats.ping_packets.entry(dst_ip).or_default() += 1;
                    }
                }
            }
    ...
    for (src_ip, stats) in source_stats {
                ...
                if protocol_for_ip_pair(src_ip, *dst_ip, flows) == Some(6) && // TCP
                   port_set.len() >= scan_threshold {
                    ...
                    if let Some(syn_count) = stats.syn_packets.get(dst_ip) {
                        if *syn_count >= port_set.len() as u32 / 2 { // At least half of connections were SYN
                            ...
                            if let Some(flow_id) = find_flow_id_for_scan(src_ip, *dst_ip, flows) {
                                ...
                                debug!("Detected TCP SYN scan from {} to {} ({} ports)",
                                       src_ip, dst_ip, port_set.len());
                            }
                        }
                    }
                }
                   ...
                   port_set.len() >= scan_threshold {
                    ...
                    if let Some(established_ports) = stats.successful_connections.get(dst_ip) {
                        ...
                        if established_ports.len() >= scan_threshold {
                            // Find the flow ID for this potential scan
                            if let Some(flow_id) = find_flow_id_for_scan(src_ip, *dst_ip, flows) {
                                attack_flows.insert(flow_id, AttackType::TcpConnectScan.to_string());
                                debug!("Detected TCP Connect scan from {} to {} ({} ports)",
                                       src_ip, dst_ip, established_ports.len());
                            }
                        }
                    }
                }
                ...
                if protocol_for_ip_pair(src_ip, *dst_ip, flows) == Some(17) && // UDP
                   port_set.len() >= udp_scan_threshold {
                    ...
                    if let Some(flow_id) = find_flow_id_for_scan(src_ip, *dst_ip, flows) {
                        ...
                        debug!("Detected UDP scan from {} to {} ({} ports)",
                               src_ip, dst_ip, port_set.len());
                    }
                }
            }
        ...
        if config.heuristics.syn_flood.enabled {
            ...
            let window_secs = chrono::Duration::seconds(config.heuristics.syn_flood.time_window_secs as i64);
            ...
            for ((dst_ip, dst_port), timestamps) in &stats.syn_timestamps {
                ...
                while window_end < sorted_timestamps.len() {
                    ...
                    while window_end < sorted_timestamps.len() &&
                          sorted_timestamps[window_end].signed_duration_since(sorted_timestamps[window_start]) <= window_secs {
                        window_end += 1;
                    }
                ...
                if highest_rate >= threshold as usize {
                    ...
                    if let Some(flow_id) = find_flow_id_for_service(src_ip, *dst_ip, *dst_port, flows) {
                        attack_flows.insert(flow_id, AttackType::SynFlood.to_string());
                        debug!("Detected SYN flood from {} to {}:{} ({} SYNs in {} seconds)",
                               src_ip, dst_ip, dst_port, highest_rate, config.heuristics.syn_flood.time_window_secs);
                    }
                }
            }
        }
            ...
            for (port, dst_ips) in &stats.dst_ips_per_port {
                ...
                if let Some(dst_ip) = dst_ips.iter().next() {
                    if let Some(flow_id) = find_flow_id_for_service(src_ip, *dst_ip, *port, flows) {
                        ...
                        debug!("Detected port sweep from {} on port {} ({} destinations)",
                               src_ip, port, dst_ips.len());
                    }
                }
            }
        }
            ...
            if stats.ping_packets.len() >= ping_sweep_threshold {
                ...
                if let Some(dst_ip) = stats.ping_packets.keys().next() {
                    if let Some(flow_id) = find_flow_id_for_ping(src_ip, *dst_ip, flows) {
                        ...
                        debug!("Detected ping sweep from {} to {} different targets",
                               src_ip, stats.ping_packets.len());
                    }
                }
            }
        }
...
fn protocol_for_ip_pair(ip_a: IpAddr, ip_b: IpAddr, flows: &HashMap<FlowKey, FlowData>) -> Option<u8> {
    for (key, _) in flows {
...
fn find_flow_id_for_scan(src_ip: IpAddr, dst_ip: IpAddr, flows: &HashMap<FlowKey, FlowData>) -> Option<String> {
    for (key, _) in flows {
        // We want to find any flow between these two IPs to associate the scan with
        if (key.ip_a == src_ip && key.ip_b == dst_ip) || 
           (key.ip_a == dst_ip && key.ip_b == src_ip) {
...
fn find_flow_id_for_service(src_ip: IpAddr, dst_ip: IpAddr, dst_port: u16, flows: &HashMap<FlowKey, FlowData>) -> Option<String> {
    for (key, _) in flows {
...
fn find_flow_id_for_ping(src_ip: IpAddr, dst_ip: IpAddr, flows: &HashMap<FlowKey, FlowData>) -> Option<String> {
...
fn parse_suricata_alerts(path: &Path) -> Result<Vec<SuricataAlert>, Box<dyn Error>> {
    ...
    info!("Processing {} EVE JSON lines in parallel", content.lines().count());
    ...
    lines.par_iter().for_each(|line| {
        if let Ok(json_value) = serde_json::from_str::<Value>(line) {
            if json_value["event_type"] == "alert" {
                ...
                let timestamp = json_value["timestamp"].as_str().unwrap_or_default().to_string();
                
                // Extract alert details
                let signature_id = json_value["alert"]["signature_id"].as_u64().unwrap_or(0) as u32;
                let signature = json_value["alert"]["signature"].as_str().unwrap_or("Unknown").to_string();
                let category = json_value["alert"]["category"].as_str().unwrap_or("Unknown").to_string();
                ...
                // Parse IP addresses
                let src_ip = src_ip_str.parse::<IpAddr>().unwrap_or_else(|_| "0.0.0.0".parse().unwrap());
                let dst_ip = dst_ip_str.parse::<IpAddr>().unwrap_or_else(|_| "0.0.0.0".parse().unwrap());
                ...
                let proto = json_value["proto"].as_u64().map(|p| p as u8);
...
fn correlate_suricata_alerts(flows: &mut HashMap<FlowKey, FlowData>, alerts: &[SuricataAlert], config: &SuricataConfig) -> usize {
    let match_count = AtomicUsize::new(0);
    
    // Group alerts by IP pairs for faster matching
    info!("Organizing {} alerts for efficient correlation", alerts.len());
    let mut alerts_by_ip: HashMap<(IpAddr, IpAddr), Vec<&SuricataAlert>> = HashMap::new();
    for alert in alerts {
        alerts_by_ip.entry((alert.src_ip, alert.dst_ip)).or_default().push(alert);
        // Also add reverse direction for bidirectional matching
        alerts_by_ip.entry((alert.dst_ip, alert.src_ip)).or_default().push(alert);
    }
    
    // Convert flows to vec for parallel processing
    let flow_entries: Vec<_> = flows.iter_mut().collect();
    
    // Process flow entries in parallel
    flow_entries.par_iter().for_each(|(key, flow_data)| {
        // Check if we have any alerts for this IP pair
        if let Some(relevant_alerts) = alerts_by_ip.get(&(key.ip_a, key.ip_b)) {
            for alert in relevant_alerts {
                // Optimization: check faster conditions first
                
                // Protocol match (if alert has protocol information)
                ...
                // If all criteria match, add the alert to the flow
                if ip_match {
                    flow_data.suricata_alerts.push((*alert).clone());
                    ...
                    if flow_data.attack_type.is_none() {
                        ...
                        let alert_str = config.alert_format
